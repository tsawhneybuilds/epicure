...
import { Send, Bot, User, Mic, MicOff, X, Plus } from 'lucide-react';
...
export function ChatModal({ isOpen, onClose, onUpdatePreferences, chatHistory, onChatUpdate, onStartNewChat }: ChatModalProps) {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      // Update the parent component with the new chat history
      if (onChatUpdate) {
  ...
  const generateAIResponse = (userInput: string): { content: string; preferences?: any } => {
    ...
    // Context-aware follow-up responses based on chat history
    if (messages.length > 2) {
      const contextualResponses = [
        "Got it! I'm refining your search based on what you just told me. The updated results should be even better!",
        "Perfect! I've updated your recommendations to better match what you're looking for.",
        "Thanks for the additional details! Your feed is now more personalized to your current needs.",
        "I understand! Let me adjust the restaurant suggestions to better fit your preferences.",
        "Excellent! I'm updating your search to show more relevant options based on our conversation."
      ];
      
      return {
        content: contextualResponses[Math.floor(Math.random() * contextualResponses.length)],
        preferences: { conversationCount: (messages.length + 1) / 2, followUp: true }
      };
    }
  ...
  const getContextualSuggestions = () => {
    if (messages.length <= 2) {
      ...
      return [
        "Actually, something cheaper would be better",
        "Can you show me healthier options?",
        "I'd prefer something closer to me",
        "Maybe something with more protein?",
        "What about vegetarian alternatives?"
      ];
    }
  };

  const suggestedQuestions = getContextualSuggestions();
      ...
      <DialogContent className="max-w-md h-[80vh] flex flex-col p-0">
        <DialogHeader className="p-4 border-b">
          <div className="flex items-center justify-between">
            <DialogTitle>
              {chatHistory && chatHistory.length > 0 ? 'Refine Your Search' : 'Personalize Your Feed'}
            </DialogTitle>
            <div className="flex items-center gap-2">
              {chatHistory && chatHistory.length > 0 && onStartNewChat && (
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={() => {
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            {chatHistory && chatHistory.length > 0 
              ? 'Continue the conversation to refine your restaurant recommendations'
